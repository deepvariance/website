    ...
    <div class="auth-page">
      <div class="auth-brand-panel">
        <div class="auth-brand-inner">
          ...
          <div>
            <p class="auth-headline">Faster inference<br>starts here.</p>
            <p class="auth-sub">Set up in minutes.<br>No GPU expertise needed.</p>
          </div>
          <div class="auth-stats">
            <div class="auth-stat"><span class="auth-stat-val">5</span><span class="auth-stat-label">models live</span></div>
            <div class="auth-stat"><span class="auth-stat-val">$25</span><span class="auth-stat-label">free credits</span></div>
          </div>
        </div>
      </div>
      ...
      <div class="auth-form-panel">
        <div class="auth-form-inner">
          ...
          <p class="auth-caption">Have an account? <a routerLink="/login" class="auth-link">Sign in &rarr;</a></p>
          ...
          <form (ngSubmit)="submit()" class="auth-fields">
            <div class="auth-field">
              ...
              <input type="text" name="name" [(ngModel)]="name" required placeholder="Your name" class="auth-input" />
            </div>
            <div class="auth-field">
              <label class="auth-label">Email</label>
              <input type="email" name="email" [(ngModel)]="email" required placeholder="you@example.com" class="auth-input" />
            </div>
            <div class="auth-field">
              <label class="auth-label">Password</label>
              <div class="auth-input-wrap">
                <input [type]="showPass() ? 'text' : 'password'" name="password" [(ngModel)]="password" required placeholder="Min. 8 characters" class="auth-input" />
            ...
            <button type="submit" class="btn-primary btn--lg auth-submit" [disabled]="loading()">
              @if (loading()) { Creating account\u2026 } @else { Create account <lucide-icon [img]="ArrowRight" [size]="14" /> }
            </button>
          </form>
          <p class="auth-demo-note">Demo: enter any details to continue</p>
        </div>
      </div>
    </div>
  `,
